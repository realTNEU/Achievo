...
func (s *Storage) createIndexes() error {
	...
	_, err = sessions.Indexes().CreateOne(context.Background(), mongo.IndexModel{
		...
		Options: options.Index().SetExpireAfterSeconds(90 * 24 * 60 * 60).SetName("sessions_ttl_idx"),
	})
	if err != nil {
		return err
	}
	...
	ruleMetadata := s.database.Collection("rule_metadata")
	_, err = ruleMetadata.Indexes().CreateMany(context.Background(), []mongo.IndexModel{
		{
			Keys:    bson.D{{Key: "game_id", Value: 1}},
			Options: options.Index().SetName("rule_metadata_game_id_idx"),
		},
		{
			Keys:    bson.D{{Key: "is_auto_generated", Value: 1}, {Key: "is_reviewed", Value: 1}},
			Options: options.Index().SetName("rule_metadata_auto_reviewed_idx"),
		},
	})
	if err != nil {
		return err
...
// SaveDetectedGame saves a newly detected game (alias for SaveGame with logging context)
func (s *Storage) SaveDetectedGame(game *models.Game) error {
	return s.SaveGame(game)
}

// SaveRuleMetadata saves or updates rule metadata
func (s *Storage) SaveRuleMetadata(metadata *models.RuleMetadata) error {
	collection := s.database.Collection("rule_metadata")
...
// GetRuleMetadata retrieves rule metadata by game ID
func (s *Storage) GetRuleMetadata(gameID string) (*models.RuleMetadata, error) {
	collection := s.database.Collection("rule_metadata")

	var metadata models.RuleMetadata
	err := collection.FindOne(s.ctx, bson.M{"game_id": gameID}).Decode(&metadata)
	if err == mongo.ErrNoDocuments {
		return nil, nil
	}
	if err != nil {
...
// ListAutoGeneratedRules retrieves all auto-generated rules
func (s *Storage) ListAutoGeneratedRules(reviewedOnly bool) ([]models.RuleMetadata, error) {
	collection := s.database.Collection("rule_metadata")

	filter := bson.M{"is_auto_generated": true}
	if reviewedOnly {
	...
	var metadata []models.RuleMetadata
	if err := cursor.All(s.ctx, &metadata); err != nil {
		return nil, err
	}
	return metadata, nil
}

// MarkRuleAsReviewed marks a rule as reviewed
func (s *Storage) MarkRuleAsReviewed(gameID string) error {
	return s.MarkRuleReviewed(gameID)
}

// MarkRuleReviewed marks a rule as reviewed (alias for consistency)
func (s *Storage) MarkRuleReviewed(gameID string) error {
	collection := s.database.Collection("rule_metadata")
...
// FetchRulesForGame retrieves rule metadata for a specific game
func (s *Storage) FetchRulesForGame(gameID string) (*models.RuleMetadata, error) {
	return s.GetRuleMetadata(gameID)
}
