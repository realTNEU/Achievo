...
func (s *Service) Start() error {
	...
	log.Println("[INFO] Starting Achievo service...")

	// Perform initial game discovery
	if s.config.Detection.DiscoveryEnabled {
		log.Println("[INFO] Discovering games on mounted drives...")
		if err := s.discoverGames(); err != nil {
			log.Printf("[WARN] Game discovery failed: %v", err)
		}
	} else {
		log.Println("[INFO] Game discovery is disabled in configuration")
	}
...
func (s *Service) loadGameRules() error {
	...
	if err != nil {
		...
		for _, entry := range autoEntries {
			...
			if err := s.ruleEngine.LoadRules(path); err != nil {
				log.Printf("[WARN] Rule file validation failed or parse error for auto-generated %s: %v", path, err)
				// Skip invalid files but continue loading others
				continue
			}
...
func (s *Service) discoverGames() error {
	...
	for _, discGame := range discovered {
		...
		// Save game to storage using SaveDetectedGame
		if err := s.storage.SaveDetectedGame(game); err != nil {
			log.Printf("[ERROR] DB connection or save failed for game %s: %v", game.Name, err)
			continue
		}
		newGamesCount++
		log.Printf("[INFO] Game discovered: %s | ID: %s | Type: %s | Saved to database", game.Name, game.ID, game.Type)
		...
		if _, err := os.Stat(autoPath); os.IsNotExist(err) {
			...
			newRulesCount++
			log.Printf("[INFO] Game discovered: %s | Generated rule file: %s", game.Name, autoPath)
			...
			if err := s.ruleEngine.LoadRules(autoPath); err != nil {
				log.Printf("[WARN] Failed to load generated rules for %s (validation or parse error): %v", game.Name, err)
	...
	log.Printf("[INFO] Discovery summary: %d new games saved, %d new rule files generated", newGamesCount, newRulesCount)
	return nil
}
